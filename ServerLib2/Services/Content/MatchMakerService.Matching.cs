...
namespace ServerLib.Services.Content
{
    public partial class MatchMakerService
    {
        public class MatchingImpl
        {
            ...
            {
                if (_matchedInProcess == 0 && adds.Count == 0 && dels.Count == 0)
                {
                ...
                ClassUtils.Swap(ref _tempList, ref _sortedTime);
            }
            ...
            public void ProcessMatch(TimeT64 now, Action<IEnumerable<MatchingNode>> onMatched)
            {
                _matchedInProcess = 0;
                matchCtx.Init(now);
