...
namespace ServerLib.Services.Core
{
    ...
    public class ServerTimerQueueService : ISingletonService, IDisposable
    {
        ...
        class TimerNode
        {
            public ITimerObject Obj { get; init; }
            public TimeT64 DueTime { get; set; }
        ...
        public ServerTimerQueueService(ILogger<ServerTimerQueueService> logger)
        {
            ...
            var dummyNode = new TimerNode { Obj = dummy, DueTime = 0 };
        ...
        public async Task<bool> BeginTimerThreadAsync(IEnumerable<ITimerObject> objs)
        {
            lock (_lock)
            {
                ...
                {
                    if (!_nodes.TryAdd(obj.TimerId, new TimerNode { Obj = obj, DueTime = 0 }))
                    {
        ...
        void Process()
        {
            ...
            while (!_shutdownRequested)
            {
                ...
                while (!_shutdownRequested)
                {
                    ...
                    Task.Run(() =>
                    {
                        dueObj.OnTimer(TimeEx.GetTimeT64());
                        OnComplete(dueObj);
                    });
                }
            }
        ...
        void OnComplete(ITimerObject obj)
        {
            ...
            lock (_lock)
            {
                ...
                var now = TimeEx.GetTimeT64();
                node.DueTime = now + obj.PeriordMs;

                if (node.DueTime < _sortedNodes.First().DueTime)
                {
                    isTop = true;
                }
